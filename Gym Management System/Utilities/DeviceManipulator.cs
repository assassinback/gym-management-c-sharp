...
namespace Gym_Management_System
{
    internal class DeviceManipulator
    {
        ...
        public bool PushUserDataToDevice(ZkemClient objZkeeper, int machineNumber, string enrollNo,string username,string password)
        {
                ...
                for (int i = 0; i < lstUserInfo.Count; i++)
                {
                    ...
                    if (sdwEnrollNumber != sLastEnrollNumber)
                    {
                        if (objZkeeper.SSR_SetUserInfo(machineNumber, sdwEnrollNumber, sName, sPassword, iPrivilege, bEnabled))//upload user information to the memory
                            objZkeeper.SetUserTmpExStr(machineNumber, sdwEnrollNumber, 1, iFlag, "");//upload templates information to the memory
                        else return false;
                    }
                    else
                    {
